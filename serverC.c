...
int main()
{
	...
	printf("\nThe Server C is up and running using UDP on port 23539.");
        fflush(stdout);
...
//receive data from AWS
	while (1)
	{
          ...
          printf("The Server C has received %d numbers",i-1);
          fflush(stdout);
          break;
...
//calculate result
        if (strcmp(recv_data,"sum")==0)
            {result=0;
             ...
             printf("The Server C has successfully finished the reduction %s: %d",recv_data,result);       
             fflush(stdout);}
